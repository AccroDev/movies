...
<template>
    <section class="max-w-3xl mx-auto p-2 md:p-0">
        ...
        <div class="containerShops flex flex-wrap">
            <ShopCard @ShowAddModal="updateShop" v-for="shop in authStore.userShops" :key="shop.id" :shop="shop" />
        </div>
        ...
        <transition name="slide">
            <AddShops  :shop="shopSelected" :key="'one'" v-if="showAddShop" @closeAddModal="CloseAddModal" />
        </transition>
    </section>
</template>
